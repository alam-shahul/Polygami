...
// computeCorners takes an N * M matrix of natural numbers, where an n represent n extruded 1x1x1 cubes
...
function computeCorners(pixelMatrix) {
    ...
    var deepCopy = [];
    var deepUneditedCopy = [];
    var maxHeight = 0;
    for(var i = 0; i < N; i++) {
        deepCopy.push([]);
        for(var j = 0; j < M; j++) {
            deepCopy[i].push(pixelMatrix[i][j]);
            deepEditedCopy[i].push(pixelMatrix[i][j]);
            maxHeight = max(pixelMatrix[i][j], maxHeight);
        }
    }

    var cumulativeGrid = new Grid(N, M);
    ...
    while(maxHeight > 0) {
        ...
        for(var corner in cumulativeGrid.corners) {
            grid.corners[corner] = cumulativeGrid.corners[corner];
        }
        ...
        for(var i = 0; i < oldN; i++) {
            ...
            for(var j = 0; j < oldM; j++) {
                ...
                newPixelMatrix[i + cumulativeRowPadding[i]][j + cumulativeColumnPadding[j]] = max(0, pixelMatrix[i][j] - 1);
    ...
    // console.log(corner);
    pixelMatrix = deepUneditedCopy;
    return cumulativeGrid;
}
...
// Takes in a 3d-matrix and projects the highest point on the z axis on each (x, y) coordinate
function projection(grid) {
    ...
    for(var i = 0; i < X; i++) {
        ...
        for(var j = 0; j < Y; j++) {
            ...
            project[i].push(0);
            for(var k = 0; k < Z; k++) {
                // TODO Enter that into the projection
                if(grid[i][j][k] === 1) {
                    project[i][j] = k;
                }
            }
